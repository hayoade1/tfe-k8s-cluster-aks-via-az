...
resource "null_resource" "aks_cluster" {
  provisioner "local-exec" {
    command = "az aks create --resource-group ${var.resource_group_name} --name ${var.cluster_name} --dns-name-prefix ${var.dns_prefix}  --agent-count ${var.agent_count} --agent-vm-size ${var.vm_size} --admin-username ${var.admin_user} --kubernetes-version ${var.kubernetes_version}  --tags 'Environment=${var.environment}' > aks_cluster.json"
  }
}
